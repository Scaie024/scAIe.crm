...
class MessengerService:
    def __init__(self):
        ...
        self.token = os.getenv('MESSENGER_TOKEN')
    ...
    async def process_message(self, message: str, user_id: str, username: str = None) -> Dict[str, Any]:
        """
        Process incoming Facebook Messenger message and generate response.
        
        Args:
            message: Incoming message text
            user_id: Facebook Messenger user ID
        ...
        try:
            ...
            print(f"Error processing Facebook Messenger message: {e}")
            error_response = {
                'success': False,
                'error': str(e),
    ...
    def _get_or_create_contact(self, db: Session, user_id: str, username: str = None) -> Contact:
        ...
        # Try to find contact by Facebook Messenger user ID
        contact = db.query(Contact).filter(Contact.facebook_messenger_user_id == user_id).first()
        
        if not contact:
            # Create new contact
            contact = Contact(
                name=username if username else f"Usuario Messenger {user_id[:10]}",
                facebook_messenger_user_id=user_id,
                platform=PlatformType.FACEBOOK_MESSENGER
            )
            db.add(contact)
            db.commit()
            db.refresh(contact)
        elif not contact.facebook_messenger_user_id:
            # Update existing contact with Messenger ID
            contact.facebook_messenger_user_id = user_id
            contact.platform = PlatformType.FACEBOOK_MESSENGER
            db.commit()
    ...
    def _get_or_create_conversation(self, db: Session, contact_id: int) -> Conversation:
        ...
        conversation = db.query(Conversation).filter(
            ...
            Conversation.platform == PlatformType.FACEBOOK_MESSENGER
        ).first()
    ...
    async def send_message(self, message: str, recipient_id: str) -> bool:
        ...
        try:
            ...
            print(f"Error sending Facebook Messenger message: {e}")
            return False
        ...
        try:
            ...
            if mode and token:
                if mode == 'subscribe' and token == verify_token:
                    print("Facebook Messenger webhook verified successfully")
                    return True
        
            return False
        except Exception as e:
            print(f"Error verifying Facebook Messenger webhook: {e}")
            return False