...
async def get_dashboard_metrics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Cálculo de tasa de conversión (contactos interesados / contactos totales)
        try:
            interested_contacts = db.query(Contact).filter(
                Contact.interest_level == InterestLevel.INTERESTED
            ).count()
        except:
            interested_contacts = 0
        
        conversion_rate = (interested_contacts / total_contacts * 100) if total_contacts > 0 else 0
        
        # Simulación de citas agendadas (en una implementación real vendría de integración con calendario)
        scheduled_appointments = interested_contacts // 3 if interested_contacts > 0 else 0
