...
def list_contacts(
    ...
    interest: Optional[str] = Query(None, description="Filter by interest level: nuevo|contactado|interesado|confirmado|no_interesado"),
    db: Session = Depends(get_db)
):
    """
    List contacts with optional search and pagination.
    """
    return contact_service.get_contacts(db, skip=skip, limit=limit, search=search, interest=interest)
...
def create_contact(
    ...
    Create new contact.
    """
    # Service handles normalization and de-duplication
