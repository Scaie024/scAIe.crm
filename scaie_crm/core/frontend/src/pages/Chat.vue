        ...
        <div class="card flex-1 flex flex-col mb-6">
          ...
          <div ref="chatContainer" class="flex-1 overflow-y-auto p-4 bg-gray-50">
            ...
            >
              ...
              >
                <div class="flex items-center mb-1" v-if="message.sender === 'agent'">
...
export default {
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        const response = await chatService.sendMessage(message, {
          name: customerInfo.value.name || undefined,
          company: customerInfo.value.company || undefined,
          email: customerInfo.value.email || undefined,
  ...
  addMessage('Lo siento, estoy teniendo problemas para responder en este momento. Por favor, inténtalo de nuevo.', 'agent');
      }
    };
    ...
    const resetConversation = () => {
      messages.value = [];
      errorMessage.value = '';
  addMessage('¡Hola! Soy tu asistente virtual de SCAIE. ¿En qué puedo ayudarte hoy?', 'agent');
    };
    
    onMounted(() => {
      // Add welcome message
  addMessage('¡Hola! Soy SCAI. Te ayudo a automatizar procesos con IA. ¿Qué te gustaría saber?', 'agent');
    });
