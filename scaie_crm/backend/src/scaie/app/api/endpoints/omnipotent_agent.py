...
@router.post("/process-message", response_model=AgentResponse)
async def process_incoming_message(
    request: IncomingMessage,
    db: Session = Depends(get_db)
):
    """
    Procesa un mensaje entrante y determina las acciones apropiadas.
    """
    try:
        # Asegurarse de que contact_info tenga la estructura correcta
        if not isinstance(request.contact_info, dict):
            raise HTTPException(status_code=400, detail="contact_info debe ser un diccionario")
...
async def execute_pending_actions(
    ...
    """
    try:
        results = omnipotent_agent.execute_pending_actions(conversation_id)
...
async def get_contact_summary(
    ...
    """
    try:
        summary = omnipotent_agent.get_contact_summary(contact_id)
        if "error" in summary:
...
async def search_contacts(
    ...
    """
    try:
        results = omnipotent_agent.search_contacts(query)
        return {"results": results}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
